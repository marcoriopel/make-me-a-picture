...
export class LobbyManagerService {
    ...
    create(req: Request, res: Response, next: NextFunction){
        ...
        switch(req.gameType) {
            case GameType.CLASSIC:
                this.lobbies.set(lobbyId, new ClassicLobby(GameType.CLASSIC, req.gameName));
                break;
            case GameType.SOLO:
                this.lobbies.set(lobbyId, new SoloLobby(GameType.CLASSIC, req.gameName));
                break;
            case GameType.COOP:
                this.lobbies.set(lobbyId, new CoopLobby(GameType.CLASSIC, req.gameName));
                break;
            default:
    ...
    join(req: Request, res: Response, username: string, next: NextFunction): void {
        var lobby: Lobby = this.lobbies.get(req.lobbyId);
        lobby.addPlayer(username);
        this.dispatchNewPlayer(username, req.lobbyId);
        next();
    }
